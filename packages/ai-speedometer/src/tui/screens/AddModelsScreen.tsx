...
export function AddModelsScreen() {
  ...
  useKeyboard((key) => {
    ...
    if (step === 'pick') {
      if (key.name === 'escape' || key.sequence === 'q' || key.sequence === 'Q') { navigate('model-menu'); return }
      ...
      if (key.name === 'return' || key.name === 'enter') {
        ...
        if (prov) { setSelectedProvider(prov); setModelInput(''); setAddedModels([]); setSaveError(''); setInputError(''); setStep('add') }
        return
      }
    }
    if (step === 'add') {
      if (key.name === 'escape') { setStep('pick'); return }
      if (key.name === 'return' || key.name === 'enter') {
        if (modelInput.trim()) { addModel() }
        else { if (addedModels.length === 0) { setInputError('Add at least one model'); return }; finishAdding() }
        return
      }
  ...
  async function addModel() {
    ...
    setInputError(''); setSaveError('')
    const name = modelInput.trim()
    try {
      const { addModelToCustomProvider } = await import('@ai-speedometer/core/ai-config')
      await addModelToCustomProvider(selectedProvider.id, { id: name.toLowerCase().replace(/[^a-z0-9-]/g, '-'), name })
      setAddedModels(ms => [...ms, name]); setModelInput('')
      ...
      <box flexDirection="column" flexGrow={1} alignItems="center" justifyContent="center">
        <box flexDirection="column" border borderStyle="rounded" borderColor={theme.border} backgroundColor={theme.background} width={CARD_W}>
          <box flexDirection="column" paddingLeft={2} paddingRight={2} paddingTop={1} paddingBottom={1}>
            <box height={1}><text fg={theme.success}>Done! Added {addedModels.length} model{addedModels.length !== 1 ? 's' : ''} to {selectedProvider?.name}.</text></box>
      ...
      <box flexDirection="column" flexGrow={1} alignItems="center" justifyContent="center">
        <box flexDirection="column" border borderStyle="rounded" borderColor={theme.border} backgroundColor={theme.background} width={CARD_W}>
          <box height={1} paddingLeft={2} paddingTop={1} flexDirection="row">
            <text fg={theme.accent}>Add Models to: </text>
          ...
          <box flexDirection="column" paddingLeft={2} paddingRight={2} paddingTop={1} paddingBottom={1}>
            {(selectedProvider.models.length > 0 || addedModels.length > 0) && (
              <box flexDirection="column">
                <box height={1}><text fg={theme.primary}>Current models:</text></box>
                {selectedProvider.models.map(m => <box key={m.id} height={1}><text fg={theme.dim}>  · {m.name || m.id}</text></box>)}
                {addedModels.map(m => <box key={m} height={1}><text fg={theme.success}>  · {m} (new)</text></box>)}
              </box>
            )}
            ...
            {addedModels.length > 0 && <box height={1}><text fg={theme.success}>Added {addedModels.length} model{addedModels.length !== 1 ? 's' : ''} so far</text></box>}
            {saveError && <box height={1}><text fg={theme.error}>Error: {saveError}</text></box>}
            {inputError && <box height={1}><text fg={theme.error}>{inputError}</text></box>}
          </box>
        </box>
      </box>
    )
  ...
  return (
    <box flexDirection="column" flexGrow={1} alignItems="center" justifyContent="center">
      <box flexDirection="column" border borderStyle="rounded" borderColor={theme.border} backgroundColor={theme.background} width={CARD_W}>
        <box height={1} paddingLeft={2} paddingTop={1}><text fg={theme.accent}>Add Models to Provider</text></box>
        <box height={1} backgroundColor={theme.border} />
        <box flexDirection="column" paddingLeft={2} paddingRight={2} paddingTop={1} paddingBottom={1}>
          {loadError && <box height={1}><text fg={theme.error}>Error: {loadError}</text></box>}
          {!loadError && providers.length === 0 && <box height={1}><text fg={theme.warning}>No custom providers yet. Add one first.</text></box>}
          {providers.map((prov, i) => {
            ...
            return (
              <box key={prov.id} height={1} width="100%" flexDirection="row" backgroundColor={isActive ? theme.border : 'transparent'}>
                ...
                <text fg={isActive ? theme.text : theme.dim} width={Math.floor((CARD_W - 8) * 0.6)}>{prov.name}</text>
                <text fg={isActive ? theme.primary : theme.border} width={Math.floor((CARD_W - 8) * 0.3)}>{prov.models.length} models</text>
